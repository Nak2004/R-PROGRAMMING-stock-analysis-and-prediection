...
server <- function(input, output, session) {
  ...
  stock_data <- reactive({
    ...
    # Define essential columns (common across datasets)
    essential_columns <- c("Date", "Close")
    
    # Define expected columns (can vary across datasets)
    ...
    missing_columns <- setdiff(essential_columns, colnames(data))
    ...
    data$Date <- as.Date(data$Date, tryFormats = c("%Y-%m-%d", "%d/%m/%Y", "%m/%d/%Y"))
  ...
  model_data <- reactive({
    ...
    data <- stock_data() %>%
      mutate(
        SMA_50 = if ("Close" %in% colnames(data)) SMA(Close, n = 50) else NA,
        EMA_20 = if ("Close" %in% colnames(data)) EMA(Close, n = 20) else NA,
        Percent_Change = if ("Close" %in% colnames(data)) 
          (Close - lag(Close)) / lag(Close) * 100 else NA
      )
    data <- data %>% drop_na() # Remove rows with missing values
    data
  })
  ...
  output$prediction_plot <- renderPlot({
    ...
    if (nrow(model_data()) < 2) {
      ...
      return() # Stop the execution if there is insufficient data
    }
